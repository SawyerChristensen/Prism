...
//
//  Album art transitions only ever play in lockstep with a real projectM preset transition - see
//  the preset-transition album-art effect section below.
//
...
// getting brighter here" vector, used by the backgroundDetail layer's distortion.
...
// off the source image via warp/zoomBlur's own center-relative overhang (see
// preset_transition_warp_blend/preset_transition_zoom_blur_blend below) - a pixel fades out
// *because of* how far that overhang has carried it, not an independent dissolve mask running
// alongside it.
...
fragment float4 projectm_composite_fragment(ProjectMPassthroughVaryings in [[stage_in]],
                                             ...
                                             texture2d<float> oldBackgroundDetailArtTexture [[texture(5)]],
                                             texture2d<float> oldSubjectArtTexture [[texture(6)]],
                                             texture2d<float> oldTextArtTexture [[texture(7)]],
                                             constant AlbumArtUniforms &u [[buffer(0)]])
{
    ...
    float warpOverhangFade = warpActive
        ...
        : 0.0;
    float bound = max(beatZoomBound, max(kWarpOverhangBound * warpOverhangFade, zoomBlurOverhangBound));
    ...
    // to a fixed horizontal split (before rotation) wherever the local gradient is too weak to give
    // a reliable direction. Aberration is sampled
    ...
    return float4(mix(wave.rgb, color, saturate(alpha)), 1.0);
}
